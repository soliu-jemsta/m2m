loadCaseApprovalComponent = function () {
	if (MainApplication.cachedState.mode) {
		whenCaseApprovalLoaded();
	} else {
		setTimeout(function () {
			MainApplication.cachedState.pageStateCall = loadCaseApprovalComponent;
		}, 1000);
	}
};
...
MainApplication.CaseApprovalComponent.ApplicationDetails = function () {
...
function whenCaseApprovalLoaded() {
	console.log("Case Approval Page Loaded successfully...");
}

MainApplication.CaseApprovalComponent.buildRating = function (rowId, key, data) {
...
MainApplication.CaseApprovalComponent.confirmSubmit = function (action) {
	MainApplication.confirmAction = MainApplication.CaseApprovalComponent.actionConfirmed;
	$("#confirmModal").modal("show");
};

MainApplication.CaseApprovalComponent.actionConfirmed = function () {
	MainApplication.CaseApprovalComponent.saveDataToList();
};

MainApplication.CaseApprovalComponent.saveDataToList = function () {
	...
	if ($spcontext.checkPassedValidation()) {
		...
		MainApplication.CaseApprovalComponent.proceedToList(formData, false);
	} else {
...
MainApplication.CaseApprovalComponent.proceedToList = function (formData) {
		...
		if (AppRequest.itemId == null) {
			vbContext.createItems([formData], globalDefinitions.stageDefinitions.listname, function (createdItemsProperties) {
				...
				vbContext.updateItems([updateObj], globalDefinitions.stageDefinitions.listname, function () {
					...
					MainApplication.CaseApprovalComponent.clearAllAttachments("SupportingDocuments", "fu");
					// $spcontext.redirect("#/", false);
					globalDefinitions.closeLoader();
...
MainApplication.CaseApprovalComponent.dateHandler = function () {
...
MainApplication.CaseApprovalComponent.deleteRowAttachment = function (elementBindProperty, index, elementId) {
    ...
    // Re-render UI
    MainApplication.CaseApprovalComponent.renderAttachments(
        elementBindProperty,
        property,
        elementId
    );
...
MainApplication.CaseApprovalComponent.clearAllAttachments = function (elementBindProperty, elementId) {
...
MainApplication.CaseApprovalComponent.renderAttachments = function (elementBindProperty, property, elementId) {
...
MainApplication.CaseApprovalComponent.recoverListData = function () {
...
MainApplication.CaseApprovalComponent.getRandomItem = function (array) {
    const randomIndex = Math.floor(Math.random() * array.length);
    return array[randomIndex];
}

MainApplication.CaseApprovalComponent.renderRandomHeader = function () {
    ...
    const randomItem = MainApplication.CaseApprovalComponent.getRandomItem(data);
